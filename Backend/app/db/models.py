...
class Company(Base):
    ...
    tokens: Mapped[list["AuthToken"]] = relationship(back_populates="company")


class HRManager(Base):
    __tablename__ = "hr_manager"

    hr_id: Mapped[int] = mapped_column(primary_key=True, index=True)
...
class Job(Base):
    ...
    hr_id: Mapped[int] = mapped_column(ForeignKey("hr_manager.hr_id"))
...
class Notification(Base):
    ...
    hr_id: Mapped[int] = mapped_column(ForeignKey("hr_manager.hr_id"), nullable=True)
...
class AuthToken(Base):
    ...
    # Either belongs to a Company or an HR 
